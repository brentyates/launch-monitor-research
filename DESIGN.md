...
- **Global multi-frame fit (`SPIN_METHOD=global`)** — ACCEPTED, current best, and the result that closed the software→hardware loop. Search one shared (axis, per-frame rotation) maximizing summed dense NCC across ALL consecutive pairs at once; the constant-spin constraint over-constrains the fit and disambiguates *large* per-frame rotations (a wrong rotation can fake-match one pair but not all). Works up to **~37°/frame**. **At 1800 fps it solves 7000 rpm (2.9%) and 11000 rpm (3.1%)**, 3000 rpm ~7% (useful); above ~40°/frame (1500 fps @ 11000 rpm = 44°) it still degrades.
  - **Loop closed (partially):** relaxed the spin-camera fps requirement from ~4–5 kfps to **~1800 fps — inside the 1300–1800 fps commercial overhead range.** lm-spin-sweep uses it via `SPIN_METHOD=global`; lm-rig still uses the old search.
- **Cheap-fps wall (500–800 fps).** Budget reality is 500–800 fps (top end only at small crops), not 1800. Swept the global fit at 500–800 fps, tight (70 px, 2–3 frames) and wide (37 px, 4–7 frames). **Nothing robustly passes across the spin range, and more frames / different zoom did not rescue it.** Structural reason: at high spin the per-frame rotation (52–114°) destroys consecutive-frame overlap regardless of frame count or zoom, and frame counts are too low for marker-periodicity to span a rotation at low spin. Occasional "passes" (e.g. 500 fps/7000 rpm = 4.2%) are aliasing coincidences. **Finding: with current CV, 500–800 fps cannot measure the full golf spin range** — consistent with commercial monitors needing 1300–1800 fps. **Key nuance by club:** spin is *low* where it matters most — **driver ≈ 2000–3000 rpm (~18–22°/frame at 800 fps → borderline reachable, ~8%)**, while irons/wedges (5000–11000 rpm) are the hard wall. Remaining levers: a learned estimator (ML), or accept a spin-rate ceiling.
- **Learned spin estimator (ML) — WORKS at cheap fps.** The renderer is a perfect-label data factory (`blender/gen_dataset.py`, EEVEE for ~10× faster gen); `ml/` is a PyTorch pipeline (uv venv, MPS on M4 Max). Model: early-fusion CNN (frames as channels so conv1 sees the inter-frame rotation) → regress rpm + axis with fps as input. Per-frame-CNN+GRU underfit (couldn't see rotation); early fusion fixed it. Trained on shots at **500–1000 fps** (the cheap regime): val error fell as the data pool grew — 844→31%, 1500→14%, **4300 shots → 9% mean / 7% median rpm, axis 0.70°**. Still data-bound (train 2.7% vs val 9% → more data keeps closing it; ≤5% looks reachable). **vs 50–300% for every hand-built CV method at the same fps — a 10–40× improvement, and axis already beats the "good" bar.** Conclusion: **ML is the path to spin at a buildable (≤800–1800 fps) frame rate.** **Transfer VALIDATED:** the EEVEE-trained model, run on a held-out **Cycles** test set (the realistic engine, never trained on), gives **rpm 8.3% median / axis 0.43° median — uniform across driver/iron/wedge (wedge 9.1% / 0.58°)**, essentially matching the EEVEE val numbers. No domain collapse (augmentation bridged the engine gap), and it holds for *high spin* — the exact regime where every CV method failed 50–300%. So at the 500–800 fps budget: **spin rate ~8% (useful, approaching good), spin axis ~0.43° (well past good), validated on realistic renders.** Remaining: more data toward ≤5% rate (still data-bound); eventually sim-to-real. The software→hardware loop closed once more — cheap hardware was infeasible for classical CV, a learned model brought it back in reach.

## Side-mount finding (Square Golf) + anti-idealization methodology

Real-world anchor: **Square Golf** — a $700 side-mounted device doing spin at **240 fps**, ball ~48 cm to the side at ball height, **27-dot marked balls**, claimed ~1% spin. This reframed the spin conclusion:

- **The "spin needs 1300–1800 fps" result was an artifact of the overhead geometry + consecutive-correlation algorithm.** Absolute-orientation per frame (resolve the marks, match to the known ball) has a **360°/frame** aliasing limit = 14,400 rpm at 240 fps — above the golf range, so no aliasing. The binding constraint is **ball pixels** (resolve the markings), which a *close side* camera provides cheaply.
- **Modeled it in sim** (`gen_dataset.py --cam/--aim/--light-energy`): close side camera + **device-driven IR lighting** (a light co-located with the camera over dark ambient — front-lit ball, markings high-contrast, shadow hidden — replacing the earlier, unfaithful overhead-sun lighting). At 240 fps the ML recovers spin at **~7% median / 2.3° axis** (clean ceiling). So a cheap 240 fps side camera *does* spin, where the overhead rig couldn't.

**Anti-idealization methodology (to avoid an over-rosy sim → bad hardware buy):**
1. The clean sim number is a **ceiling, not a forecast** — it omits sensor noise/gain, bit-depth, detection error (we crop with GT center), marking wear, ambient IR, calibration error, residual strobe blur.
